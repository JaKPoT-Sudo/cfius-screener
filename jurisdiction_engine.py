"""
jurisdiction_engine.py — the deterministic CFIUS decision tree.

This module is pure functions over structured input: no database, no web, no
Claude. Given the facts of a transaction, it walks the jurisdictional tests of
31 CFR Part 800 in order and returns a Determination whose findings trail
shows every step, its answer, and the regulation that drives it.
...
it wrong. A language model must never make that call. Claude's jobs (from
Milestone 2) are upstream — turning a plain-English deal description into the
structured facts this engine consumes — and downstream — writing the memo
narrative ABOUT these conclusions. The conclusions themselves come from here,
the same answer every time.

The tree, in plain English:

  1. Is the acquirer a foreign person?          no → CFIUS has no jurisdiction
  2. Is the target a US business?               no → CFIUS has no jurisdiction
  3. Is the target a TID US business?           (feeds steps 5 and 7)
  4. Does the deal give the foreign person CONTROL?
        yes → covered control transaction (jurisdiction attaches, even for
              excepted investors)
  5. If not control: is it a covered INVESTMENT?
        = TID business + (board seat / observer / non-public technical
          information / substantive decision-making role)
        Excepted investors are carved out of THIS test entirely.
  6. Neither?                                    → CFIUS has no jurisdiction
  7. If covered: is a declaration MANDATORY?
        a. substantial-interest prong: a foreign government holds ≥49% of the
           acquirer AND the acquirer takes ≥25% voting interest in a TID
           business
        b. critical-technology prong: the target makes critical technologies
           that would need a US export authorization for the acquirer's home
           country
        Excepted investors are exempt from both prongs.
"""
