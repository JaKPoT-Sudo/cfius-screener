"""risk_engine.py — deterministic national security risk scoring.

No web, no database, no Claude. Every weight comes from config.py.
...
Three analytic dimensions — Threat / Vulnerability / Consequence:
  ...
  THREAT        — How adversarial is the acquirer and its principals?
                  (acquirer country, state ownership of acquirer)

  VULNERABILITY — How much sensitive access does this deal confer?
                  (what the acquirer can see, control, or exploit)

  CONSEQUENCE   — How damaging would exploitation be at a national level?
                  (what a hostile actor could do with that access)
...
from jurisdiction_engine import Determination, TransactionFacts

# Tier labels in ascending order
