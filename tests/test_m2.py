"""Tests for Milestone 2 — AI intake, memo, and PDF routes.

All Claude calls are mocked — no test makes a network request.
"""
...
# ---------------------------------------------------------------------------
# claude_intake module
# ---------------------------------------------------------------------------

def test_parse_deal_description_success():
    """parse_deal_description returns a dict with required keys."""
    ...
    with patch("claude_intake.anthropic.Anthropic") as mock_cls:
        mock_cls.return_value.messages.create.return_value = fake_response
        with patch("claude_intake.ANTHROPIC_API_KEY", "sk-test"):
            from claude_intake import parse_deal_description
...
def test_parse_deal_description_no_api_key():
    ...
    with patch("claude_intake.ANTHROPIC_API_KEY", ""):
...
def test_draft_memo_success():
    ...
    try:
        ...
        with patch("claude_memo.anthropic.Anthropic") as mock_cls:
            mock_cls.return_value.messages.create.return_value = fake_response
            with patch("claude_memo.ANTHROPIC_API_KEY", "sk-test"):
                from claude_memo import draft_memo
...
def test_draft_memo_no_api_key():
    ...
    try:
        ...
        with patch("claude_memo.ANTHROPIC_API_KEY", ""):
...
def test_intake_post_calls_claude_and_shows_confirm():
    with patch("main.parse_deal_description", return_value=_fake_parse_result()) as mock_parse:
        with TestClient(app) as client:
...
def test_intake_post_claude_error_rerenders_form():
    from claude_intake import IntakeError
    with patch("main.parse_deal_description", side_effect=IntakeError("API down")):
        with TestClient(app) as client:
            resp = client.post("/intake", data={"description": "some deal"})
    assert resp.status_code == 200
...
def test_intake_confirm_runs_engine_and_redirects():
    with TestClient(app) as client:
        resp = client.post("/intake/confirm", data={
    ...
    assert "/screening/" in resp.headers["location"]
...
def test_generate_memo_stores_and_redirects():
    ...
    with patch("main.draft_memo", return_value=FAKE_MEMO):
        with TestClient(app) as client:
            resp = client.post(f"/screening/{sid}/memo", follow_redirects=False)
...
def test_generate_memo_claude_failure_redirects_with_error():
    ...
    from claude_memo import MemoError
    with patch("main.draft_memo", side_effect=MemoError("API down")):
        with TestClient(app) as client:
            resp = client.post(f"/screening/{sid}/memo", follow_redirects=False)
...
def test_memo_pdf_returns_pdf():
    ...
    with TestClient(app) as client:
        resp = client.get(f"/screening/{sid}/memo.pdf")
